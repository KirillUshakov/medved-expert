<?php
	...
	class WT_Catalog {
		...
		public $pagehook;
		public $pageslug = 'catalog_settings';
		
		private $post_type = 'wt_element';
		private $slug = 'wt_catalog';
		
		private $rewrite_slug = 'catalog';
		...
		private function __construct() {
			...
			//Register post type Elements
			add_action( 'init', array( &$this, 'register_post_type' ) );
			//Add taxonomy filter to admin page list of elements
			...
			//Prepare screen for catalog settings page
			add_filter( 'screen_layout_columns', array( &$this, 'on_screen_layout_columns' ), 10, 2 );
			//Call functions to add catalog settings page
		...
		public function register_post_type() {
			$labels = array(
				'name' => _x( 'Elements', 'General name for post type', 'walnut' ),
				'singular_name' => _x( 'Element', 'Single name for post type', 'walnut' ),
				'add_new' => _x( 'Add New', 'Text for button add a case', 'walnut' ),
				'add_new_item' => _x( 'Add New Element', 'Title for page of adding post', 'walnut' ),
				'edit_item' => _x( 'Edit Element', 'Title for page of editing post', 'walnut' ),
				'new_item' => _x( 'New Element', 'Text of new post', 'walnut' ),
				'view_item' => _x( 'View Element', 'Text for viewing post', 'walnut' ),
				'search_items' => _x( 'Search Elements', 'Text for searching post', 'walnut' ),
				'not_found' => _x( 'No Elements yet', 'Text if no elements', 'walnut' ),
				'not_found_in_trash' => _x( 'No Elements in trash', 'Text if no photos in trash', 'walnut' ),
				'parent_item_colon' => '', 'all_items' => _x( 'Elements', 'Name of menu', 'walnut' ),
				'menu_name' => _x( 'Catalog', 'Name of menu', 'walnut' ),
			);
			
			register_post_type( $this->post_type, array(
				'labels' => $labels,
				'description' => _x( 'Elements for creating elements page', 'Description of post type', 'walnut' ),
		...
		function post_link( $permalink, $post, $leavename ) {
			...
			if( isset( $options[ 'remove_slug_catalog' ] ) && $options[ 'remove_slug_catalog' ] ) {
		...
		public function on_admin_menu() {
			$this->pagehook = add_submenu_page( 'edit.php?post_type=' . $this->post_type,
				_x( 'Catalog settings', 'Title of settings page', 'walnut' ),
		...
		public function on_load_settings_page() {
			...
			add_meta_box( 'wt_catalog_main', _x( 'Main settings', 'Title for meta box', 'walnut' ), array(
		...
		private function save_settings() {
			...
			if( isset( $_POST[ 'display_type' ] ) &&
				in_array( $_POST[ 'display_type' ], array( '', 'sections', 'elements' ) ) ) {
				$valid[ 'display_type' ] = $_POST[ 'display_type' ];
			}
			update_option( 'wt_catalog_settings', $valid );
		}
		
		public function add_meta_box_settings_main() {
			do_settings_sections( 'wt_catalog_main' );
		}
		
		public function catalog_settings() {
			add_settings_section( 'wt_catalog_main', '', array( &$this, 'section_main' ), 'wt_catalog_main' );
			
			add_settings_field( 'display_type', _x( 'Select display type', 'Label for field', 'walnut' ),
				array( &$this, 'field_display_type' ), 'wt_catalog_main', 'wt_catalog_main' );
			add_settings_field( 'description', _x( 'Catalog page description', 'Label for field', 'walnut' ),
				array( &$this, 'field_description' ), 'wt_catalog_main', 'wt_catalog_main' );
			
			register_setting( 'wt_catalog_settings', 'display_type' );
			register_setting( 'wt_catalog_settings', 'description' );
			
			register_setting( 'wt_catalog_settings', 'wt_catalog_settings' );
		}
		...
		private function get_template( $section, $setting, $data ) {
			if( file_exists( get_template_directory() . '/core/tmpl/metaboxes/wt_catalog_settings/' . $section . '/' .
				$setting . '.php' ) ) {
				include get_template_directory() . '/core/tmpl/metaboxes/wt_catalog_settings/' . $section . '/' .
					$setting . '.php';
			}
		}
		...
		public function admin_bar_links( $admin_bar ) {
			if( !is_admin() && get_query_var( 'post_type' ) == $this->post_type ) {
				$admin_bar->add_menu( array(
					...
					'title' => _x( 'Edit Catalog Settings', 'Name of admin bar menu', 'walnut' ),
			...
			if( is_admin() && get_current_screen()->base == $this->post_type . '_page_' . $this->pageslug ) {
				$admin_bar->add_menu( array(
					'id' => 'wp-admin-bar-edit',
					'title' => _x( 'View Catalog Page', 'Name of admin bar menu', 'walnut' ),
