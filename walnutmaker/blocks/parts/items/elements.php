<?php
	...
	if( $all ) {
		$terms = get_sub_field( 'catalog-sections', $obj );
		$items = new WP_Query( array(
			'post_type' => 'wt_element', 'posts_per_page' => get_option( 'posts_per_page' ),
			'paged' => get_query_var( 'paged' ) ? get_query_var( 'paged' ) : 1, 'tax_query' => array(
				array( 'taxonomy' => 'wt_catalog', 'field' => 'id', 'terms' => $terms ),
			),
		) );
	} else {
		$items = get_sub_field( 'elements', $obj );
	}
	if( $all ) {
		if( $items->have_posts() ) {
			//Pagination fix
			...
			while( $items->have_posts() ) {
				$items->the_post();
				get_template_part( 'blocks/parts/items/inc/element' );
			}
			echo '</ul>';
